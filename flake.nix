    ...
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        linuxPackages = with pkgs; [
          ...
          alsa-plugins
          udev
          # Vulkan driver and deps (so loader can load ICDs; avoids host /usr libs)
          libdrm
          zlib
          mesa
        ];
        ...
        platformShellHook = if isLinux then ''
          ...
          # Prefer Nix Mesa Vulkan ICDs so vulkaninfo/bevy find drivers (libdrm, zlib, etc. from Nix)
          export XDG_DATA_DIRS=${pkgs.mesa}/share:''${XDG_DATA_DIRS:-/usr/share}
          ''
        else
          "";
